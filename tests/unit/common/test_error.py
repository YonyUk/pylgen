...
class TestError:
    ...
    def test_error_creation(self,error_type:ErrorType,line:int,column:int,msg:str):
        error = Error(error_type,line,column,line,column + 1,msg)

        assert error.type == error_type
        assert error.start_position == (line,column)
        assert error.end_position == (line,column+1)
    ...
    def test_semantic_error(self):
        error = SemanticError('nada',1,1,1,2)
        assert error.start_position == (1,1)
        assert error.end_position == (1,2)
    ...
    def test_errors_comparision(self):
        err1 = LexicalError('nada',1,1,1,2)
        err2 = LexicalError('nada',1,1,1,2)

        assert err1 == err2
        assert hash(err1) == hash(err2)

        err2 = LexicalError('nada1',1,1,1,2)

        assert err1 != err2
        assert hash(err1) != hash(err2)

        err2 = LexicalError('nada',1,1,1,3)

        assert err1 != err2
        assert hash(err1) != hash(err2)

        err2 = SyntaxError('nada',1,1,1,2)

        assert err1 != err2
        assert hash(err1) != hash(err2)

        err2 = SemanticError('nada',1,1,1,2)

        assert err1 != err2
        assert hash(err1) != hash(err2)

        err2 = RuntimeError([],1,1,1,2,'nada')
