...
[![PyPI 0.7.0](https://img.shields.io/pypi/v/pylgen-core.svg)](https://pypi.org/project/pylgen-core?cache=1)
...
*What makes PyLGEN different from other parser generators or compiler frameworks?*
 ...
 - **Parsing**: an **LALR(1)** engine with conflict detection and **panic-mode error recovery**. It lets your DSL **process malformed input without stopping**.
...
Building a language should be **fun, educational and productive**. That's why PyLGEN focuses on:
 ...
 - **Performance**: The framework core is **optimized in Cython**. You can **compile your own Cython extensions** if you need more performance.

> ## Who is it for?

 - **Students and teachers**: PyLGEN follows the classic compiler course flow, but with hands-on approach.
 - **Software engineers**: For creating DSLs, configuration languages, or code analysis tools without being locked into monolithic generators.
 - **Curious explorers**: For experimenting and understanding how a compiler works under the hood.
...
> ## Where to Go Next?
 ...
 - **[Under the hood](api/examples/intro.md)**: Build a lexer and parser from scratch, using the same building blocks that the framework uses internally.
 - **[Examples](examples/intro.md):** See practical examples of using PyLGEN to build interpreters and DSLs.
 - **[Table of Contents](table_of_contents.md)**: Browse the full documentation, organized by topic.
